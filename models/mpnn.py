...
class Message(nn.Module):
    ...
    def forward(self, edge_features, node_features, receivers, senders):
        message_inputs = torch.cat(
            [edge_features, node_features[receivers], node_features[senders]], dim=1
        )
...
class InteractionLayer(nn.Module):
    def __init__(self, reduce_fn, message_dim, hidden_dim, out_dim, layers):
        super().__init__()
...
class MPNN(nn.Module):
    def __init__(
        ...
        self.node_encoder = MLP(
            feature_sizes=[node_dim, latent_dim], activation="gelu", activate_final=True
        )

        self.edge_encoder = MLP(
            feature_sizes=[16, latent_dim],
            activation="gelu",
            activate_final=True,
        )
    ...
    def forward(self, node_features, node_positions, batch_idx, **kwargs):
        ...
        edge_features = self.edge_encoder(
            node_positions[edge_index[1]] - node_positions[edge_index[0]]
        )
