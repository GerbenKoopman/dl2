...
MEAN_HALOS_DICT = {
    ...
    "y": 500.0947802559321,
    "z": 499.964508664328,
    "J_x": 212560050888254.06,
    "J_y": 349712732356652.25,
    "J_z": -100259775332585.12,
...
STD_HALOS_DICT = {
    ...
    "J_x": 2.4294356933448945e18,
    "J_y": 2.3490019110577966e18,
    "J_z": 2.406422979830857e18,
...
def _parse_function(
    ...
    feature_tensors = [
        tf.io.parse_tensor(parsed_features[k], out_type=tf.float32) for k in features
    ]
    param_tensors = [
        tf.io.parse_tensor(parsed_params[k], out_type=tf.float32) for k in params
    ]

    # Stack the feature tensors to create a single tensor
    # Each tensor must have the same shape
    stacked_features = tf.stack(
        feature_tensors, axis=1
    )  # Creates a [num_points, num_features] tensor
...
def get_halo_dataset(
    ...
    dataset = dataset.map(
        partial(
            _parse_function, features=features, params=params, include_tpcf=include_tpcf
        )
    )
    ...
    if standardize:
        if include_tpcf:
            ...
            dataset = dataset.map(
                lambda x, p: ((x - mean) / std, (p - mean_params) / std_params)
            )
...
class CosmologyDataset(Dataset):
    ...
    def convert_to_torch(self, dataset, num_data):
        ...
        elif self.task == "geo_erwin":
            pos, target = [], []
            for i, (x, p) in enumerate(tqdm(dataset)):
                pos.append(x.numpy())
                target.append(p.numpy())
                if i == num_data:
                    break

        return [torch.from_numpy(x).squeeze() for x in pos], [
            torch.from_numpy(x).squeeze() for x in target
        ]
    ...
    def collate_fn(self, batch: list):
        ...
        batch["edge_index"] = knn_graph(
            batch["pos"], k=self.knn, batch=batch["batch_idx"]
        )
        return batch
