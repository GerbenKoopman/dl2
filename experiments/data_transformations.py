...
class RotatedCosmologyDataset:
    ...
    def __init__(self, base_dataset, device='cpu', seed=0):
        ...
        self.rotation = torch.tensor(self.rotation.as_matrix(), dtype=torch.float32, device=self.device)
        ...
        if self.determinant == -1:
            ...
            self.rotation[0, :] = -self.rotation[0, :]  # Flip first row (equivalent to reflecting through yz-plane)
